 ...
 * @param {boolean} options.centerOnMount - Si es true, centra el elemento en la pantalla al montarse
 */
export function useDraggableResizable({
  ...
  // Calcular posición inicial: si centerOnMount, centrar usando las dimensiones conocidas
  const [position, setPositionState] = useState(() => {
    if (centerOnMount) {
      const w = initialWidth || 600;
      const h = initialHeight || 600;
      return {
        x: Math.max(0, Math.floor((window.innerWidth - w) / 2)),
        y: Math.max(0, Math.floor((window.innerHeight - h) / 2)),
      };
    }
    return { x: initialX ?? 0, y: initialY ?? 0 };
  });
