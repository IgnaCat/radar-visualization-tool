...
def build_W_operator(
    ...
    for iz, n_pairs, temp_file in results:
        ...
        try:
            ...
            logger.error(f"Error cargando archivo temporal {temp_file}: {e}")
            # Intentar eliminar archivo corrupto
            try:
                os.remove(temp_file)
            except:
                pass
            # Intentar reconstruir el nivel secuencialmente
            logger.info(f"Reconstruyendo nivel {iz} secuencialmente...")
            # Recrear args para este nivel
            z_coord = z_coords[iz]
            args = (
                iz,
                z_coord,
            ...
            iz_rebuilt, n_pairs_rebuilt, temp_file_rebuilt = _process_single_level(args)
            data = np.load(temp_file_rebuilt)
            level_indptr = data["indptr"]
            level_indices = data["gate_indices"]
            level_weights = data["weights"]
            n_pairs = n_pairs_rebuilt
            temp_file = temp_file_rebuilt
