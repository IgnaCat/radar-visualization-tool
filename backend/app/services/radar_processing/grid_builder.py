...
from .grid_geometry import infer_blind_range_m, infer_last_gate_range_m
...
def get_or_build_W_operator(
    ...
    if last_gate_range_m is None:
        last_gate_range_m = infer_last_gate_range_m(radar_to_use)
    ...
    with lock:
        ...
        W = build_W_operator(
            ...
            last_gate_range_m=last_gate_range_m,
        ...
        metadata = {
            ...
            "last_gate_range_m": float(last_gate_range_m),
            "nnz": W.nnz,
            "shape": W.shape,
...
def get_or_build_grid3d_with_operator(
    ...
    W = get_or_build_W_operator(
        ...
        grid_shape=grid_shape,
        grid_limits=grid_limits,
        h_factor=h_factor,  # None = usar adaptativos
        nb=nb,
        bsp=bsp,
        min_radius=min_radius,
        toa=toa,
    ...
    visual_gatefilter = build_gatefilter_for_visual(
        radar_to_use, visual_filters, field_to_use
    )
    ...
    x_coords = np.linspace(grid_limits[2][0], grid_limits[2][1], grid_shape[2]).astype(
        np.float32
    )
    y_coords = np.linspace(grid_limits[1][0], grid_limits[1][1], grid_shape[1]).astype(
        np.float32
    )
    z_coords = np.linspace(grid_limits[0][0], grid_limits[0][1], grid_shape[0]).astype(
        np.float32
    )
