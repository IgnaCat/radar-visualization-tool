...
def calcule_radial_angle(radar_lat, radar_lon, punto_lat, punto_lon):
    ...
    y = np.cos(radar_lat_rad) * np.sin(punto_lat_rad) - (
        np.sin(radar_lat_rad) * np.cos(punto_lat_rad) * np.cos(d_lon)
    )
...
def variable_radar_cross_section(
    ...
    radar_range_km = round(
        volumen_radar_data.ngates
        * volumen_radar_data.range["meters_between_gates"]
        / 1000
    )
    ...
    with rasterio.open(tif_path) as src:
        ...
        with WarpedVRT(src, resampling=Resampling.nearest, add_alpha=False) as vrt:
            ...
            perfil_elevacion = np.fromiter(
                (v[0] for v in vrt.sample(coords)), dtype=np.float32, count=len(coords)
            )
    ...
    distances = np.array(
        [
            geodesic((start_lat, start_lon), (lat_points[i], lon_points[i])).km
    ...
    with rasterio.open(tif_path) as src:
        with WarpedVRT(src, resampling=Resampling.nearest, add_alpha=False) as vrt:
            ...
            elevacion_interes = np.float32(
                np.nan if (vrt.nodata is not None and val == vrt.nodata) else val
            )
    ...
    ax2.text(
        0.98,
        0.95,
        f"Ángulo Radial: {radial_angle:.2f}°",
...
def generate_pseudo_rhi_png(
    ...
    relative_url = (
        f"static/tmp/{session_id}/{unique_out_name}"
        if session_id
        else f"static/tmp/{unique_out_name}"
    )
    ...
    if _km_between(site_lon, site_lat, end_lon, end_lat) > range_max_km:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"El punto está fuera del alcance ({range_max_km:.1f} km).",
        )
    if start_lon is not None and start_lat is not None:
        ...
        if dstart > range_max_km:
            raise HTTPException(
                status_code=400,
                detail=f"El punto de inicio está fuera del alcance ({range_max_km:.1f} km).",
            )
    ...
    if start_lon is not None and start_lat is not None:
        ...
        try:
            ...
            start_lon_f = float(start_lon)
        same_origin = (
            geodesic((start_lat_f, start_lon_f), (site_lat, site_lon)).meters <= 1000.0
        )
        if not same_origin:
...
def _generate_segment_transect_png(
    *,
    ...
    try:
        radar_name, estrategia, volume, _ = extract_metadata_from_filename(
            Path(filepath).name
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"No se pudo extraer metadata del archivo: {e}"
        )
