...
class StatsOrchestrator:
    ...
    @staticmethod
    def reproject_polygon(polygon_gj_4326: dict, target_crs: pyproj.CRS) -> dict:
        """
    ...
    @staticmethod
    def calculate_stats(
        arr: np.ma.MaskedArray, polygon_mask: np.ndarray
    ) -> Optional[Dict]:
        """
    ...
    def compute_stats_from_cache(
        cache_key: str, polygon_gj_4326: dict, filters: List = [], field: str = ""
    ) -> Dict:
        """
        ...
        poly_mask = geometry_mask(
            [gj_dst], invert=True, out_shape=arr.shape, transform=transform
        )
    ...
    def process_stats_request(payload: RadarStatsRequest) -> RadarStatsResponse:
        ...
        stats_result = StatsOrchestrator.compute_stats_from_cache(
            cache_key, payload.polygon_geojson, payload.filters or [], field
        )

        # 6. Si no está cacheado, generar grilla bajo demanda
        if (
            stats_result.get("noCoverage")
            and stats_result.get("reason") == "No cacheado"
        ):
